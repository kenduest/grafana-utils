...
class DashboardScreenshotHelperTests(unittest.TestCase):
    def test_dashboard_screenshot_capture_via_devtools_uses_stitch_for_full_page_png(
        self,
    ):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                ...
                mocked_launch.return_value.__enter__.return_value = (
                    object(),
    ...
    def test_dashboard_screenshot_infer_output_format_rejects_unknown_extension(self):
        with self.assertRaisesRegex(
            GrafanaError, "Unable to infer screenshot output format"
        ):
    ...
    def test_dashboard_screenshot_parse_vars_query_splits_vars_and_passthrough_pairs(
        self,
    ):
    ...
    def test_dashboard_screenshot_validate_screenshot_args_requires_target(self):
        ...
        with self.assertRaisesRegex(
            GrafanaError, "Set --dashboard-uid or pass --dashboard-url"
        ):
            screenshot.validate_screenshot_args(args)

    def test_dashboard_screenshot_validate_screenshot_args_rejects_invalid_dimensions(
        self,
    ):
        args = screenshot.make_screenshot_args(
            dashboard_uid="cpu-main",
    ...
    def test_dashboard_screenshot_validate_screenshot_args_rejects_tiles_without_full_page(
        self,
    ):
        args = screenshot.make_screenshot_args(
            dashboard_uid="cpu-main",
        ...
        with self.assertRaisesRegex(
            GrafanaError, "--full-page-output tiles or manifest requires --full-page"
        ):
            screenshot.validate_screenshot_args(args)
    ...
    def test_dashboard_screenshot_build_dashboard_capture_url_merges_dashboard_url_and_overrides(
        self,
    ):
        args = screenshot.make_screenshot_args(
            dashboard_url=(
    ...
    def test_dashboard_screenshot_capture_dashboard_screenshot_requires_backend(self):
        ...
        with self.assertRaisesRegex(
            GrafanaError, "requires a browser backend or a configured Grafana client"
        ):
            screenshot.capture_dashboard_screenshot(args)

    def test_dashboard_screenshot_capture_dashboard_screenshot_passes_normalized_request_to_backend(
        self,
    ):
        captured = {}
    ...
    def test_dashboard_screenshot_capture_dashboard_screenshot_resolves_slug_from_live_dashboard(
        self,
    ):
        captured = {}
        ...
        screenshot.capture_dashboard_screenshot(
            args, backend=backend, client=FakeClient()
        )
        self.assertIn("/d/cpu-main/cpu-overview", captured["url"])

    def test_dashboard_screenshot_capture_dashboard_screenshot_uses_browser_backend_by_default(
        self,
    ):
        with tempfile.TemporaryDirectory() as tmpdir:
    ...
    def test_dashboard_screenshot_capture_dashboard_screenshot_allows_pdf_with_browser_backend(
        self,
    ):
        args = screenshot.make_screenshot_args(
            dashboard_uid="cpu-main",
    ...
    def test_dashboard_screenshot_write_full_page_output_tiles_writes_parts_without_manifest(
        self,
    ):
        with tempfile.TemporaryDirectory() as tmpdir:
            request = {
                ...
                "segments": [
                    {
                        ...
                        "scroll_y": 0,
                        "source_top": 0,
                        "image": Image.new("RGBA", (200, 150), (1, 2, 3, 255)),
                    },
                    {
                        "index": 1,
                        "scroll_y": 140,
                        "source_top": 10,
                        "image": Image.new("RGBA", (200, 140), (4, 5, 6, 255)),
                    },
                ],
            }
            metadata = {
