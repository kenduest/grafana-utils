...
MODULE_PATH = PYTHON_ROOT / "grafana_utils" / "datasource" / "live_mutation_safe.py"
...
class DatasourceLiveMutationSafeTests(unittest.TestCase):
    ...
    def test_datasource_live_mutation_plan_add_datasource_distinguishes_existing_name(
        self,
    ):
    ...
    def test_datasource_live_mutation_plan_add_datasource_distinguishes_uid_name_mismatch(
        self,
    ):
        client = FakeDatasourceClient(
            datasources=[{"id": 1, "uid": "prom-live", "name": "Prometheus Main"}]
    ...
    def test_datasource_live_mutation_add_datasource_uses_client_helper_when_available(
        self,
    ):
    ...
    def test_datasource_live_mutation_delete_datasource_uses_client_helper_when_available(
        self,
    ):
        client = FakeDatasourceClient(
    ...
    def test_datasource_live_mutation_delete_datasource_surfaces_specific_mismatch_action(
        self,
    ):
        client = FakeDatasourceClient(
            datasources=[{"id": 7, "uid": "prom-main", "name": "Prometheus Main"}]
