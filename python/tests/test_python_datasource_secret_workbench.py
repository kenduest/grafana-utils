...
MODULE_PATH = PYTHON_ROOT / "grafana_utils" / "datasource_secret_workbench.py"
...
class DatasourceSecretWorkbenchTests(unittest.TestCase):
    ...
    def test_datasource_secret_build_datasource_secret_plan_fails_for_missing_placeholder_value(
        self,
    ):
    ...
    def test_datasource_secret_collect_secret_placeholders_rejects_opaque_secret_replay(
        self,
    ):
        with self.assertRaisesRegex(
            GrafanaError,
    ...
    def test_datasource_secret_build_datasource_secret_plan_shapes_resolved_plan_for_review(
        self,
    ):
